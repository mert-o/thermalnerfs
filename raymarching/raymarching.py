    ...
    if BACKEND is None:
        try:
            ...
            try:
                ...
                raise ImportError("Could not import _backend from either _raymarching_mob or .backend.")
...
class _composite_rays_train_ts(Function):
    @staticmethod
    @custom_fwd(cast_inputs=torch.float32)
    def forward(ctx, sigmas, rgbs, ts, rays, T_thresh=1e-4, alpha_mode=False):
        ''' composite rays' rgbs, according to the ray marching formula.
        ...
        Returns:
            ...
            weights_sum: float, [N,], the alpha channel
            depth: float, [N, ], the Depth
            image: float, [N,], the thermal array (after multiplying alpha!)
        '''
        ...
        depth = torch.empty(N, dtype=sigmas.dtype, device=sigmas.device)
        image = torch.empty(N, dtype=sigmas.dtype, device=sigmas.device)
        get_backend().composite_rays_train_ts_forward(sigmas, rgbs, ts, rays, M, N, T_thresh, alpha_mode, weights, weights_sum, depth, image)
    ...
    def backward(ctx, grad_weights, grad_weights_sum, grad_depth, grad_image):
        ...
        get_backend().composite_rays_train_ts_backward(grad_weights, grad_weights_sum, grad_depth, grad_image, sigmas, rgbs, ts, rays, weights_sum, depth, image, M, N, T_thresh, alpha_mode, grad_sigmas, grad_rgbs)

        return grad_sigmas, grad_rgbs, None, None, None, None


composite_rays_train_ts = _composite_rays_train_ts.apply

class _composite_rays_train_rgbt(Function):
    @staticmethod
    @custom_fwd(cast_inputs=torch.float32)
    def forward(ctx, sigmas, rgbs, ts, rays, T_thresh=1e-4, alpha_mode=False):
        ''' composite rays' rgbs, according to the ray marching formula.
        ...
        depth = torch.empty(N, dtype=sigmas.dtype, device=sigmas.device)
        image = torch.empty(N, 4, dtype=sigmas.dtype, device=sigmas.device)

        get_backend().composite_rays_train_rgbt_forward(sigmas, rgbs, ts, rays, M, N, T_thresh, alpha_mode, weights, weights_sum, depth, image)
    ...
    def backward(ctx, grad_weights, grad_weights_sum, grad_depth, grad_image):
        ...
        get_backend().composite_rays_train_rgbt_backward(grad_weights, grad_weights_sum, grad_depth, grad_image, sigmas, rgbs, ts, rays, weights_sum, depth, image, M, N, T_thresh, alpha_mode, grad_sigmas, grad_rgbs)

        return grad_sigmas, grad_rgbs, None, None, None, None


composite_rays_train_rgbt = _composite_rays_train_rgbt.apply

class _composite_rays_train_sc(Function):
    @staticmethod
    @custom_fwd(cast_inputs=torch.float32)
    def forward(ctx, sigmas, rgbs, temps, ts, rays, T_thresh=1e-4, alpha_mode=False):
        ''' composite rays' rgbs, according to the ray marching formula.
        Args:
            rgbs: float, [M, 3]
            sigmas: float, [M,]
        ...
        depth = torch.empty(N, dtype=sigmas.dtype, device=sigmas.device)
        image = torch.empty(N, 3, dtype=sigmas.dtype, device=sigmas.device)
        temp_image = torch.empty(N, dtype=sigmas.dtype, device=sigmas.device)

        get_backend().composite_rays_train_sc_forward(sigmas, rgbs,temps, ts, rays, M, N, T_thresh, alpha_mode, weights, weights_sum, depth, image,temp_image)

        ctx.save_for_backward(sigmas, rgbs, ts, rays, weights_sum, depth, image, temps, temp_image)
        ctx.dims = [M, N, T_thresh, alpha_mode]

        return weights, weights_sum, depth, image, temp_image
    
    @staticmethod
    @custom_bwd
    def backward(ctx, grad_weights, grad_weights_sum, grad_depth, grad_image, grad_temp_image):
        
        grad_weights = grad_weights.contiguous()
        grad_weights_sum = grad_weights_sum.contiguous()
        grad_depth = grad_depth.contiguous()
        grad_image = grad_image.contiguous()
        grad_temp_image = grad_temp_image.contiguous()

        sigmas, rgbs, ts, rays, weights_sum, depth, image, temps, temp_image = ctx.saved_tensors
        M, N, T_thresh, alpha_mode = ctx.dims
   
        grad_sigmas = torch.zeros_like(sigmas)
        grad_rgbs = torch.zeros_like(rgbs)
        grad_temps = torch.zeros_like(temps)

        get_backend().composite_rays_train_sc_backward(grad_weights, grad_weights_sum, grad_depth, grad_image,grad_temp_image, sigmas, rgbs,temps, ts, rays, weights_sum, depth, image,temp_image, M, N, T_thresh, alpha_mode, grad_sigmas, grad_rgbs,grad_temps)

        return grad_sigmas, grad_rgbs, grad_temps, None, None, None, None


composite_rays_train_sc = _composite_rays_train_sc.apply
# ----------------------------------------
# infer functions
# ----------------------------------------
...
class _composite_rays_ts(Function):
    @staticmethod
    @custom_fwd(cast_inputs=torch.float32) # need to cast sigmas & rgbs to float
    def forward(ctx, n_alive, n_step, rays_alive, rays_t, sigmas, rgbs, ts, weights_sum, depth, image, T_thresh=1e-2, alpha_mode=False):
        ''' composite rays' rgbs, according to the ray marching formula. (for inference)
        ...
        sigmas = sigmas.float().contiguous()
        rgbs = rgbs.float().contiguous()
        get_backend().composite_rays_ts(n_alive, n_step, T_thresh, alpha_mode, rays_alive, rays_t, sigmas, rgbs, ts, weights_sum, depth, image)
        return tuple()


composite_rays_ts = _composite_rays_ts.apply

class _composite_rays_rgbt(Function):
    @staticmethod
    @custom_fwd(cast_inputs=torch.float32) # need to cast sigmas & rgbs to float
    def forward(ctx, n_alive, n_step, rays_alive, rays_t, sigmas, rgbs, ts, weights_sum, depth, image, T_thresh=1e-2, alpha_mode=False):
        ''' composite rays' rgbs, according to the ray marching formula. (for inference)
        ...
        sigmas = sigmas.float().contiguous()
        rgbs = rgbs.float().contiguous()
        get_backend().composite_rays_rgbt(n_alive, n_step, T_thresh, alpha_mode, rays_alive, rays_t, sigmas, rgbs, ts, weights_sum, depth, image)
        return tuple()


composite_rays_rgbt = _composite_rays_rgbt.apply

class _composite_rays_sc(Function):
    @staticmethod
    @custom_fwd(cast_inputs=torch.float32) # need to cast sigmas & rgbs to float
    def forward(ctx, n_alive, n_step, rays_alive, rays_t, sigmas, rgbs, temps, ts, weights_sum, depth, image, temp_image, T_thresh=1e-2, alpha_mode=False):
        ''' composite rays' rgbs, according to the ray marching formula. (for inference)
        Args:
            n_alive: int, number of alive rays
            n_step: int, how many steps we march
        ...
        get_backend().composite_rays_sc(n_alive, n_step, T_thresh, alpha_mode, rays_alive, rays_t, sigmas, rgbs,temps, ts, weights_sum, depth, image,temp_image)
        return tuple()


composite_rays_sc = _composite_rays_sc.apply